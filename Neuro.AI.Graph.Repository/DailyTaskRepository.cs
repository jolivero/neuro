...
namespace Neuro.AI.Graph.Repository
{
	public class DailyTaskRepository
	{
		...
		public async Task<IEnumerable<DailyTaskOperator>> Select_dailyTask_by_userId(string currentDay, Guid userId, int? taskId = null)
		{
		...
		public async Task<IEnumerable<DailyTask>> Select_dailyTask_operator_history(int taskId, Guid userId)
		{
		...
		public async Task<string> UpdateCloseDailyTasksAsync()
		{
			const string sp = "sp_update_close_dailyTasks";

			try
			{
				var rows = await _db.ExecuteAsync(sp, commandType: CommandType.StoredProcedure);

				return $"OK | Affected records: {rows}";
			}
			catch (Exception ex)
			{
				return ex.Message;
